...
def parse_cmd_options(argv):
    ...
    parser.add_argument('--jsonfile', default=None, type=str, help='.')
    parser.add_argument('--flops', default=None, type=str, help='.')
    parser.add_argument('--evaluate_only', action='store_true', help='Only evaluation.')
    parser.add_argument('--use_test_set', action='store_true', help='Use test set for evaluation.')
    ...
    parser.add_argument('--expid', default=None, type=str, help='Experiment ID.')
    ...
    parser.add_argument('--arch_txt', type=str, default='')
    # learning rate setting
    parser.add_argument('--lr', default=0.1, type=float, help='initial learning rate per 256 batch size')
    parser.add_argument('--target_lr', default=None, type=float,
                        help='Target LR (e.g. 1e-4 for cosine). If None, uses target_lr_per_256.')
    parser.add_argument('--lr_per_256', default=0.1, type=float, help='initial learning rate per 256 batch size')
    parser.add_argument('--target_lr_per_256', default=0.0, type=float,
                        help='Target LR per 256 batch (used when target_lr is None).')
    parser.add_argument('--lr_mode', default='cosine', type=str,
                        help='LR schedule: cosine, fixed, stagedecay, linear.')
    ...
    parser.add_argument('--lr_stage_list', default=None, type=str,
                        help='Multi-step milestones (e.g. "30,60,90" for 100ep). Requires lr_mode=stagedecay.')
    parser.add_argument('--lr_stage_decay', default=None, type=float,
                        help='Multi-step decay factor (e.g. 0.1). Requires lr_mode=stagedecay.')
    ...
    parser.add_argument('--csv_file_path', default='sampled_1k_15ms_a57.csv', type=str)
    parser.add_argument('--img_per_class', default=None, type=int)
    parser.add_argument('--epfrac', default=1, type=float)
    ...
    parser.add_argument('--use_tqdm', action='store_true', help='Use tqdm to show training progress.')

    parser.add_argument('--start_epoch', default=0, type=int, help='manual epoch number (useful on restarts)')
    parser.add_argument('--epochs', default=100, type=int, metavar='N', help='number of total epochs to run')
    parser.add_argument('--max_iters', default=None, type=int,
                        help='Cap mini-batch iterations per epoch (combine with --epochs 1 for an '
                             'exact total iteration count, e.g. a quick demo run).')
    parser.add_argument('--save_dir', default='./tmp', type=str, help='where to save models.')
    parser.add_argument('--val_freq', default=1, type=int, help='How many epochs to validate.')
    ...
    parser.add_argument('--weight_init', type=str, default='xavier', help='How to initialize parameters')
    ...
    # for loading dataset
    parser.add_argument('--data_dir', type=str, default=None, help='path to ImageFolder dataset root (must contain train/ subdir)')
    parser.add_argument('--dataset', type=str, default='auto_imagefolder', help='name of the dataset')
    ...
    parser.add_argument('--num_cv_folds', type=int, default=0, help='Number of cross-validation folds.')
    parser.add_argument('--cv_id', type=int, default=0, help='Current ID of cross-validation fold.')
    parser.add_argument('--input_image_size', type=int, default=32, help='input image size.')
    parser.add_argument('--input_image_crop', type=float, default=0.875, help='crop ratio of input image')
    parser.add_argument('--stem_head_size', type=int, default=None, help='stem head size.')
    # for loading model
    parser.add_argument('--arch', default=None, help='model names/module to load')
    parser.add_argument('--plainnet_struct', type=str, default=None, help='PlainNet structure string (for ZEN model)')
    parser.add_argument('--plainnet_struct_txt', type=str, default=None, help='Path to file containing PlainNet structure (for ZEN model)')
...
class LearningRateScheduler():
    ...
    def get_lr(self, num_received_training_instances=None):
        ...
        assert stop_instances > warmup_instances, f"stop_instances: {stop_instances} > warmup_instances: {warmup_instances}, num_received_training_instances: {num_received_training_instances}, num_training_instances: {self.num_training_instances}, stop_epoch: {self.stop_epoch}, warmup_epoch: {self.warmup_epoch}"
        ...
        if self.mode == 'fixed':
            ...
            factor = (1 - math.cos(math.pi * ratio_epoch)) / 2.0
